...
class SonnenEco6ModeSelect(CoordinatorEntity[SonnenEco6Coordinator], SelectEntity):
    """Select for SetOperationMode.

    IMPORTANT: We reflect the current option directly from M06 (state).
    That means if M06 becomes 13 while charging, the select will show the label
    mapped to 13.
    """
    ...
    def __init__(
        ...
        self._attr_options = list(OP_MODES.keys())

    @property
    def current_option(self) -> str | None:
        """Return current option based on M06 numeric state."""
    ...
    async def async_select_option(self, option: str) -> None:
        """Set operation mode via SetOperationMode."""
